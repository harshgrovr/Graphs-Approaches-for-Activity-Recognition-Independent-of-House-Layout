...
from singleHouseGraphClassification import getUniqueStartIndex, getStartAndEndIndex
...
def eval_net(args, net, dataloader, criterion, run_config, house_name, text = 'train'):
    ...
    if args.save_embeddings:
        ...
        print(hiddenLayerEmbeddings.shape)
        print(np.array(all_labels).shape)
        embedding = pd.DataFrame(hiddenLayerEmbeddings)
        embedding['activity'] = np.array(all_labels)
    ...
    return loss, acc, f1, per_class_acc_dict, confusion_matrix, embedding
...
def main(args, run_config, house_name, csv_raw = None, shuffle=False):
    ...
    for train_index, test_index in loo.split(uniqueIndex):
        ...
        if run_config is 'raw':
            graph_path = os.path.join('../../../data/', house_name , house_name + '.bin')
        ...
        train_idx = list(set(np.arange(len(graphs))) - set(np.arange(start, end)))
    ...
    house_results_dictionary['total_test_per_class_accuracy'] = total_per_class_accuracy
...
if __name__ == '__main__':
    ...
    for run_config in ['raw']:
        ...
        for house_name in house_list[4]:
            print(house_name, '\n\n')
            csv_raw = os.path.join('../../../../../Research/data/' , house_name, house_name + '.csv')
            # Train and then Save embeddings
            args.save_embeddings = False
            house_results_dictionary, total_embedding = main(args, run_config, house_name,  pd.read_csv(csv_raw), shuffle=False)
            total_embedding.to_csv("../../../data/" + house_name + "/" + run_config + "_leave_one_day_graph_embeddings.csv", index=False)
            total_embedding.to_csv("../../../../../Research/data/" + house_name + "/" + run_config + "_leave_one_day_graph_embeddings.csv", index=False)

            results_list.append(house_results_dictionary)
            print('saving..... ', house_name)

            np.save(os.path.join('../../../logs/graph_classification', house_name + '_' + run_config + '_leave_one_day.npy'), results_list)
